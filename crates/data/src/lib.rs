...
//!
//! ## Статус: интерфейсы + инфраструктура авторизации (Фаза 0)
//!
//! Сетевой gRPC-клиент подключается в фазе интеграции API (требует генерации
//! стабов из `.proto`). Уже реализована и протестирована вспомогательная
//! инфраструктура, не зависящая от транспорта:
//! - [`auth`] — менеджер JWT-токена с обновлением по сроку;
//! - [`rate_limit`] — пометодный ограничитель частоты (token-bucket);
//! - [`secret`] — абстракция хранилища API-секрета (env/инъекция; keyring — за
//!   фичей в десктопной сборке).
