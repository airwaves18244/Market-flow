...
- `storage` — **движок DuckDB** (bundled): версионированные миграции, ингест
  баров/инструментов/снимков оборота, загрузка классификации секторов и
  аналитические запросы (оборот по секторам, бары за период и пр.).
- `data` — контракты (трейт `MarketData`, ошибки), классификация секторов и
  планировщик бэкфилла (нарезка диапазона на окна под лимит API).
- `app` — smoke-точка входа, прогоняющая сквозной поток `миграции → ингест →
  запрос` через in-memory DuckDB.
