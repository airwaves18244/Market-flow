...
- `domain` (Фазы 10–12) — чистые модули новых вкладок: `algo` (Super Candles,
  FUTOI, HI2, Mega Alerts), `keyactivity` (правила + LLM-промпт), `history`
  (каталог датасетов, план дозагрузки), `options` (Блэк-76/Башелье, греки, IV,
  4 модели улыбки с калибратором, конструктор стратегий).
- `frontend` — фронт-приложение (Фазы 3–12): Vite + Svelte 5 + TS, тёмная тема,
...
Вкладки V2 (Обзор · Delta · Торговля · Бэктест) и V2.5 (MOEX ALGO · Данные ·
Опционы) реализованы end-to-end на mock/локальных данных: доменные модули,
IPC-обработчики, команды Tauri и Svelte-компоненты (`MoexAlgoTab`, `HistoryTab`,
`OptionsTab` с калькулятором, улыбкой и конструктором стратегий).
...
- `ROADMAP.md` — пошаговый план развития (фазы 0–9, статус верификации).
- `ROADMAP_PHASE_10-12.md` — детальный план фаз 10–12 (MOEX ALGO · Историзация · Опционы).
- `SUMMARY.md` — живой срез статуса проекта.
- **`V3/PRD.md`** — продуктовая спецификация V3: целевое состояние терминала
  уровня state of the art (аналитика + торговля).
- **`V3/TODO.md`** — план работ V3 с распределением задач по моделям Claude
  (Fable/Opus/Sonnet/Haiku) и уровням усилий.
