...
### Фазы 10–12 — доменное ядро + App/IPC + вкладки фронта ✅ (сетевые слои — отдельной итерацией)
...
- **App/IPC + фронт**: обработчики и команды Tauri для трёх новых вкладок;
  компоненты `MoexAlgoTab`/`KeyActivityTable`/`KeyActivitySummary`,
  `HistoryTab`/`DatasetManager`, `OptionsTab`/`OptionCalculator`/`SmileView`/
  `SmileChart`/`StrategyBuilder`/`PayoffChart` — типизированный IPC + мок-режим
  (работают в браузере без бэкенда).

Вне этой итерации (нужны сеть/ключи/десктоп): транспорт `data::moex`/`http`/`llm`,
таблицы `storage` для датасетов и блоки `(verify)` — фиксация контракта
ALGOPACK по живым фикстурам.
...
## Следующее — V3
План следующего мажорного этапа вынесен в папку **`V3/`**:
- `V3/PRD.md` — детальная продуктовая спецификация терминала уровня
  state of the art (аналитика + торговля);
- `V3/TODO.md` — декомпозиция на задачи по моделям Claude и уровням усилий.

Ближайшие блокеры вне кода:
