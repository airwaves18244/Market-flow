...
### Фазы 10–12 — доменное ядро ✅ + фронт ✅ (единый интерфейс 0–12)
...
**Фронт 10–12 (эта итерация)** — новые вкладки сведены с обзорным фронтом (0–9)
в единый интерфейс по дизайн-прототипу Claude Design (`Market_Terminal.dc.html`);
ключевая логика и IPC-контракт унаследованы от текущего фронта:
- **MOEX ALGO** — тулбар (инструмент/период/рынок) + 5 модулей: «Ключевая
  активность + ИТОГО» (боевой IPC `key_activity`/`key_activity_summary`),
  «Супер-свечи» (свечи+VWAP+объём+дисбаланс), «FUTOI» (физ/юр, long/short/нетто),
  «Концентрация HI2» (ряд+зоны+ранжирование), «Мега-алёрты». Ряды 4 модулей —
  из демо-генераторов `lib/algoMock.ts` до подключения `data::moex`.
- **Опционы** — калькулятор (`option_price`/`option_implied_vol`), улыбка
  (`smile_fit`: все модели сразу, карточки параметров+RMSE, «активная» модель),
  конструктор стратегий (`strategy_eval`: пресеты-чипы, payoff, греки).
- **Данные** — загрузка истории (`history_plan`) + менеджер датасетов
  (`history_datasets`/`history_delete`).
- **Настройки** — отображение, паспорт ALGOPACK (рынки/вотчлист), конструктор
  правил Key Activity (условия + JSON, засев из `key_activity_rules`), LLM
  (провайдер/модель/ключ/токены/авто), данные, опционы. Секреты в UI не хранятся.
- Единый визуальный язык — общие классы `app.css`; проверено:
  `svelte-check` 0 ошибок · `vitest` 26 тестов · `vite build` ок · прогон в
  Chromium (все вкладки рендерятся без ошибок консоли).

Вне этой итерации (нужны сеть/ключи/десктоп): транспорт `data::moex`/`http`/`llm`,
таблицы `storage`, IPC/Tauri (`app`) для 4 демо-модулей ALGO и блоки `(verify)` —
фиксация контракта ALGOPACK по живым фикстурам; упаковка десктоп-приложения.
...
## V2 — Бэктестер, Торговля, Delta (Фаза 9)
Терминал из «обзора» превращается в «рабочую станцию» с верхними вкладками
**Обзор · Delta · Торговля · Бэктест**. Вся новая логика — чистая, в `domain`,
покрыта тестами; `app` и фронт — тонкие адаптеры, как и прежде.

- **Хранилище**: таблица `trades` (схема v2) — тиковая лента под footprint/дельту
  и заполнение симулятора (`Store::insert_trades`/`trades`, `Writer::trades`).
- **`domain::backtest`**: трейт `Strategy` (целевая позиция), детерминированный
  движок с учётом позиции/комиссии/слиппеджа, отчёт (P&L, win-rate, profit
  factor, просадка, Sharpe) и библиотека стратегий (`ma_cross`, `same_lot`,
  `iceberg`, `cvd_momentum`) по id + параметрам.
- **`domain::delta`**: footprint (объём по ценам/сторонам агрессора, дельта/CVD)
  и роботы-детекторы (равные лоты, айсберг по стакану, поглощение) — анализ и
  оверлей на графике, без торговли.
- **`domain::trading`**: симулятор исполнения (paper trading) — заявки/сделки,
  счёт и позиции (средняя цена, реализованный/нереализованный P&L), предторговый
  риск и `SimBroker` (рынок проходит стакан, лимитки стоят и исполняются на
  ленте, стопы по пробою).
- **`app`**: обработчики `list_strategies`/`run_backtest`/`delta_footprint`/
  `robot_scan` + сессия `TradeSession` (`submit_order`/`cancel_order`/
  `order_blotter`/`positions`/`account`), DTO, команды Tauri, событие `fill:tick`.
- **Фронт**: вкладки и три модуля (`Backtester`, `DeltaView`+`DeltaChart`,
  `TradePanel` с кликабельным DOM-стаканом). Типизированный IPC + мок-режим
  (включая мок-симулятор) — всё работает в браузере без бэкенда.
- **Каркас боевого роутинга**: `data::OrderRouter` + `SimOrderRouter` (по
  умолчанию), заглушка `FinamOrderRouter` за фичей `live-trading` (off);
  реальный gRPC `OrderService`/`AccountsService` — отдельная интеграция.
