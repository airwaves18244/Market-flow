...
### Фаза 9 — Дизайн-итерация: оболочка + Сводка / Бэктест / Торговля ✅ (с прототипами)

Реализована по эталону Claude Design (`frontend/design/`, см.
`frontend/design/README.md`). Ветка: `design/prototype-v2`.

**Бэкенд (Rust) — реальная, протестированная логика:**
- `domain::model::AssetClass::Fx` — новый класс актива (валютный спот).
  `code/from_code/ALL` обновлены; `cross_asset_summary`/`turnover_timeline`/
  `flow_sankey` считают четыре класса. FX-доля = 0, пока FX не ингестится.
- `domain::metrics::regime` — чистый классификатор режима рынка:
  `assess_regime(ClassFlows, threshold) -> RegimeAssessment` (Risk-ON/OFF/
  Neutral + conviction 0..100). Детерминирован, 6 юнит-тестов.
- `app::api::summary(from,to) -> RegimeSignalDto` — нетто-поток по классам из
  снимков (`class_net_flow`) → `assess_regime`; `state::summary` + Tauri-команда
  `summary`; smoke в `main.rs`. DTO: `RegimeSignalDto`, `ClassFlowDto`;
  `TurnoverByClassPoint` получил поле `fx`. Покрыто тестами (`summary_*`).

**Фронтенд (Svelte 5):**
- `App.svelte` переписан в shell по дизайну: верхняя панель (бренд, глобальный
  период `1Д…1Г`, LIVE, READ-ONLY), левый рейл из 10 рабочих пространств,
  статус-бар. Период считает окно `[fromTs,toTs]` и перезагружает аналитику.
- Новые компоненты: `SummaryPanel.svelte` (режим/conviction/нетто-потоки/
  решения/риски — на данных `ipc.summary`), `BacktestPanel.svelte` и
  `TradePanel.svelte` (UI-прототипы).
- `types`/`ipc`/`mock`/`assetClass` дополнены: тип `fx`, `RegimeSignalDto`,
  команда `summary`, мок режима и FX-доли.

**Прототипы без бэкенда (намеренно):**
- **Бэктест** — результат симулируется детерминированно в UI.
- **Торговля** — заявки отклоняются (гарантия READ-ONLY v1 сохранена).
  Их бэкенд расписан в `ROADMAP.md` (Фаза 9.2 / 9.3).

**Проверка ветки:** `cargo fmt --all --check`, `cargo clippy --workspace
-- -D warnings`, `cargo test --workspace` — зелёные (весь workspace, включая
новые `domain::metrics::regime::*` и `app::api::summary_*`). Фронт:
`npm run check` (0 ошибок, 245 файлов) и `npm run build` — зелёные.
