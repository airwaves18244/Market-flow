...
- **Доменное ядро** в `crates/domain/src/live/` (чистое, без сети/таймеров,
  полное покрытие тестами — 24 теста):
  - `book` — стакан (DOM): лучшие цены, спред, mid, дисбаланс, глубина;
  - `tape` — лента (Time & Sales): классификация сторон (явный инициатор или
    правило тика), CVD, VWAP, объём по сторонам;
  - `alerts` — правила (цена ↑/↓, |изменение|, всплеск объёма, спред) и движок
    их проверки по снимку рынка с серьёзностью (`info|warning|critical`);
  - `replay` — курсор воспроизведения по шкале времени (шаг/перемотка/прогресс).
- **API-обработчики** в `crates/app/src/api.rs` (scaffold поверх доменной
  логики, как `yield_curve`, — до интеграции живого источника):
  - `order_book()` — книга вокруг якорной цены (закрытие последнего бара);
  - `time_and_sales()` — лента из детерминированных сделок + агрегаты;
  - `active_alerts()` — снимок рынка из дневных баров → демо-правила → движок;
  - `replay_state()` — курсор по меткам времени сохранённых баров.
- **DTO** в `crates/app/src/dto.rs`: `OrderBookDto`/`OrderBookLevelDto`,
  `TimeAndSalesDto`/`TapeEntryDto`/`TapeStatsDto`, `TriggeredAlertDto`,
  `ReplayStateDto`.
- **Tauri**: команды `order_book`, `time_and_sales`, `active_alerts`,
  `replay_state` + заготовки live-push (`emit_quote_tick`, `emit_trade_tick`,
  `emit_order_book`) для будущего стрима.
- **Frontend компоненты**: `DomLadder` (лесенка с полосами глубины),
  `TimeSales` (лента + buy/sell-бар + CVD/VWAP), `AlertsPanel` (список по
  серьёзности), `ReplayControls` (play/step/seek + прогресс); мок-генераторы.
- **Тесты**: 6 новых в `app` (стакан, лента, алёрты на всплеск/тишину, replay).

## Следующее (Фаза 7 — стрим, Фаза 8)
- Фаза 7 (остаток): реальный стрим Finam (`SubscribeQuote`,
  `SubscribeLatestTrades`, `SubscribeOrderBook`) и пуш событий в вебвью.
- Фаза 8: Асинхронный планировщик ингеста, полноценный dockview, упаковка, полировка.
