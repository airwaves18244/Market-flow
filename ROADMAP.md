...
| 10 | MOEX ALGO (Super Candles · FUTOI · HI2 · Mega · Key Activity · LLM) | ✅ | — (контракты сверены живым ключом, T14 закрыт) |
...
| — | Сквозное: HTTP-слой, персист настроек, egress | ✅ | — (P1 закрыт: egress + все ключи) |
...
- ✅ Egress-allowlist окружения (P1 закрыт 2026-07-17): `apim.moex.com`,
  `iss.moex.com`, `fs.moex.com`, `data.moex.com`, `tradeapi.finam.ru`,
  `openrouter.ai`; ключи `FINAM_API_SECRET`/`MOEX_ALGO_API`/
  `OPENROUTER_API_KEY` заданы.
...
**Осталось:** — (2026-07-17: все 6 датасетов ALGOPACK и LLM-вызов сверены
живыми ответами, смоук — `example algopack_check`; исправлены пути
`futoi`/свечей, длинный формат `hi2`, дефолтная LLM-модель).
...
**Осталось:** — (оба источника сверены живьём: Finam gRPC — `live_check`,
MOEX tradestats/candles — `algopack_check`).
...
1. **P1 (пользователь)** — закрыт (2026-07-17): egress к MOEX/Finam/
   OpenRouter + все три ключа в `.env`.
2. **T14** — закрыт (2026-07-17): Finam gRPC (`live_check`), доска опционов
   ISS, все 6 датасетов ALGOPACK и живой LLM-вызов (`algopack_check`)
   сверены живыми ответами; исправлены эндпоинт Finam, пути futoi/свечей,
   длинный формат hi2, форвард доски, дефолтная LLM-модель.
