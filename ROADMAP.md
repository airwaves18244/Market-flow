...
| 10 | MOEX ALGO (Super Candles · FUTOI · HI2 · Mega · Key Activity · LLM) | ✅ | сверка ALGOPACK: нужен ключ `MOEX_ALGO_API` (T14-остаток) |
...
| — | Сквозное: HTTP-слой, персист настроек, egress | 🟡 | LLM-хост в egress + ключ `MOEX_ALGO_API` (остаток P1) |
...
- ✅ Egress-allowlist окружения (P1, 2026-07-17): `apim.moex.com`,
  `iss.moex.com`, `fs.moex.com`, `data.moex.com`, `tradeapi.finam.ru` —
  открыты. ⛔ LLM-хосты (`openrouter.ai` и др.) всё ещё закрыты (403) —
  живой LLM-вызов не сверен.
...
**Осталось:** сверка датасетов ALGOPACK боевым ключом — egress к
`apim.moex.com` открыт и эндпоинт живой (401 без ключа), но `MOEX_ALGO_API`
в окружении не задан; фикстуры ALGOPACK остаются синтетическими.
...
**Осталось:** живой прогон загрузки: Finam-источник сверен смоуком T14
(эндпоинт исправлен на `tradeapi.finam.ru`), MOEX-источник ждёт ключа
`MOEX_ALGO_API`.
...
1. **P1 (пользователь)** — выполнен частично (2026-07-17): egress к MOEX и
   Finam открыт, Finam-секрет задан. Остаток: `MOEX_ALGO_API` в `.env` +
   LLM-хост (`openrouter.ai`) в egress-allowlist.
2. **T14** — выполнен в доступной части (2026-07-17): Finam gRPC — живой
   сквозной прогон (эндпоинт исправлен на `tradeapi.finam.ru`); доска
   опционов ISS — контракт сверен, парсер/фикстуры обновлены. Остаток:
   датасеты ALGOPACK (ключ) и LLM-вызов (egress).
3. **T15**: упаковка MSI/NSIS + иконки — подготовка (иконки, bundle-конфиг,
   инструкция) в репо; сборка — на десктопе.
4. Полировка: cleanup-список из ревью PR #23 выполнен (2026-07-17).
   Вне критического пути остаются: dockview, тепловая карта риска, 12.5.1,
   реальный OrderService (вне v1).
